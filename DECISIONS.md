...
- N18 — Rate limiting on /api/subscribe via hosting firewall or email provider limits (same-origin check alone is not enough) — BLOCKING — gate: deploy. Host is Netlify (N9): /api/subscribe needs a limit of its own that is proven on the live site (point 12), not a limit that may land on a whole-app function
