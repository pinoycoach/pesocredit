...
| C115 | This compares your numbers with a published limit. It does not tell you whether the loan or lender is safe, what you owe, or what happens next. | `src/lib/copy/en.ts:181` | directly under the rows, whenever they are shown, in normal-size text (added in step F3) |
| C71 | An illustration only. The lender's disclosure statement gives the official EIR. This is not legal advice and does not refer to any specific lender. | `src/lib/copy/en.ts:183` | always, bottom of the card |

### 1.6 How we calculated this (expander)

| ID | Text | Where | Shown when |
|---|---|---|---|
| C72 | How we calculated this | `src/lib/copy/en.ts:185` | expander title |
| C73 | What you received (net proceeds) | `src/lib/copy/en.ts:187` | row label; value beside it |
| C74 | Total you pay (by day {Z}) | `src/lib/copy/en.ts:188` | row label; value beside it |
| C117 | Late penalty you entered | `src/lib/copy/en.ts:201` | row label, only when a late penalty is entered (N41); the penalty beside it |
| C75 | Total cost (interest + fees + penalty) | `src/lib/copy/en.ts:190` | row label; value beside it |
| C76 | Daily rate (EIR) | `src/lib/copy/en.ts:193` | row label; value beside it |
| C77 | Per month, simple (daily rate × 30) | `src/lib/copy/en.ts:194` | row label; value beside it |
| C78 | Per month, compounded ((1 + daily rate) ^ 30 − 1) | `src/lib/copy/en.ts:196` | row label; value beside it |
| C79 | Nominal interest per month (from your payments) | `src/lib/copy/en.ts:199` | row label; value beside it |
| C80 | {₱ total cost} · {%} of the amount borrowed | `src/lib/copy/en.ts:191` | value of the total-cost row |
| C81 | The EIR is the daily rate that balances what you received with everything you pay, not counting late penalties. The circular does not say how to turn the daily rate into a monthly one, ×30 or compounded, so we show both. | `src/lib/copy/en.ts:202` | below the rows |

### 1.7 Loan calendar

| ID | Text | Where | Shown when |
|---|---|---|---|
| C82 | Loan calendar | `src/lib/copy/en.ts:204` | card title |
| C83 | {days}-day term · {number of payments} payments | `src/lib/copy/en.ts:205` | card subtitle; with one payment: "{days}-day term · 1 payment" |
| C84 | Day 0 | `src/lib/copy/en.ts:207` | loans over 16 days: list, first row |
| C85 | Received · {₱ received} | `src/lib/copy/en.ts:208` | list, first row |
| C86 | Day {day} | `src/lib/copy/en.ts:209` | list, one row per payment |
| C87 | Payment {n} · {₱ amount} | `src/lib/copy/en.ts:210` | list, one row per payment |
| C88 | First follow-up (your note) | `src/lib/copy/en.ts:211` | list, when a follow-up day is set |
| C89 | Day | `src/lib/copy/en.ts:212` | loans of 16 days or less: day grid, every cell |
| C90 | Received | `src/lib/copy/en.ts:213` | grid cell, day 0 |
...
| ID | Text | Where | Shown when |
|---|---|---|---|
| C95 | Schedule | `src/lib/copy/en.ts:219` | card title |
| C96 | Day | `src/lib/copy/en.ts:220` | column heading |
| C97 | Payment | `src/lib/copy/en.ts:221` | column heading |
| C98 | Received | `src/lib/copy/en.ts:222` | first row (day 0) |
...
| ID | Text | Where | Shown when |
|---|---|---|---|
| C99 | Sources | `src/lib/copy/en.ts:224` | card title |
| C100 | **<u>SEC MC No. 14, s. 2025</u>** In effect from 1 April 2026. For unsecured, general-purpose loans from lending and financing companies of up to <u>₱10,000</u> and up to <u>4 months</u>. | `src/lib/copy/en.ts:227` | paragraph |
| C101 | **The limits** (the circular calls them ceilings) Nominal interest <u>6% a month</u>, EIR <u>12% a month</u>, and a total cost of no more than <u>100% of the amount borrowed</u>. | `src/lib/copy/en.ts:238` | paragraph |
| C102 | **This tool does not check** the late-penalty limit (<u>5% a month</u>), because that needs the number of days late. | `src/lib/copy/en.ts:250` | paragraph |
| C103 | **RA No. 3765 (Truth in Lending Act)** The basis for calculating the EIR, according to the circular. The circular does not say whether the monthly rate is ×30 or compounded, so we show both. | `src/lib/copy/en.ts:258` | paragraph |
...
| ID | Text | Where | Shown when |
|---|---|---|---|
| C104 | Want a free checklist, and a note when the rules change? | `src/lib/copy/en.ts:265` | card title |
| C105 | Optional. You don't need it to see your result, and none of the numbers you entered are included. | `src/lib/copy/en.ts:267` | card description |
| C106 | Email | `src/lib/copy/en.ts:268` | field label |
| C107 | I agree to get emails with the checklist and updates. I have read the <u>Privacy Policy</u>. | `src/lib/copy/en.ts:269` | consent checkbox label; the link opens /privacy (link text from privacyLinkLabel) |
| C108 | Send | `src/lib/copy/en.ts:271` | button |
| C109 | Sending… | `src/lib/copy/en.ts:272` | button, while sending |
| C110 | Thank you. We'll email you the checklist. | `src/lib/copy/en.ts:273` | replaces the form after success |
| C111 | It didn't go through. Please try again later. | `src/lib/copy/en.ts:274` | beside the button after a failure |
...
| ID | Text | Where | Shown when |
|---|---|---|---|
| C112 | Want a fuller guide? | `src/lib/copy/en.ts:275` | card text |
| C113 | Get the ₱99 guide (in Tagalog) | `src/lib/copy/en.ts:276` | link |

### 1.12 Footer

| ID | Text | Where | Shown when |
|---|---|---|---|
| C114 | Privacy Policy | `src/lib/copy/en.ts:283` | link to /privacy, new tab |
...
| ID | Text | Where | Shown when |
|---|---|---|---|
| P01 | Back to the calculator | `src/lib/copy/en.ts:282` | link back to / |
| P02 | Privacy Policy | `src/lib/copy/en.ts:54` | page title |
| P03 | DRAFT, for a lawyer to review. This is not final and is not yet our official policy. | `src/lib/copy/en.ts:281` | banner, only while PRIVACY_STATUS is draft |

### 2.2 What we collect

| ID | Text | Where | Shown when |
|---|---|---|---|
| P04 | What we collect | `src/lib/copy/en.ts:286` | section heading |
| P05 | Your email address and the time you agreed. You give these only in the optional form on the calculator, and we collect them only if you agree (the consent box is ticked). That's all. | `src/lib/copy/en.ts:288` | paragraph |

### 2.3 What we don't collect

| ID | Text | Where | Shown when |
|---|---|---|---|
| P06 | What we don't collect | `src/lib/copy/en.ts:292` | section heading |
| P07 | We don't save or send the numbers you enter. The calculation happens on your device, and those numbers are never included in the email form. | `src/lib/copy/en.ts:294` | paragraph |
| P08 | We have no accounts, and no analytics or tracking on this site. The site itself sets no cookies, and our fonts come from our own site, not from other websites. | `src/lib/copy/en.ts:295` | paragraph |
| P09 | If you tap the link to the SEC (sec.gov.ph), you leave this site, and the SEC's website handles your visit. | `src/lib/copy/en.ts:296` | paragraph |

### 2.4 Why we collect it

| ID | Text | Where | Shown when |
|---|---|---|---|
| P10 | Why we collect it | `src/lib/copy/en.ts:300` | section heading |
| P11 | To send you the free checklist and the updates about new rules that you asked for, and to keep a record that you agreed. | `src/lib/copy/en.ts:302` | paragraph |

### 2.5 Who receives your email address

| ID | Text | Where | Shown when |
|---|---|---|---|
| P12 | Who receives your email address | `src/lib/copy/en.ts:306` | section heading |
| P13 | When you send the form, our email service provider, Resend, emails your address and the time you agreed to the site's operator. The operator then adds your address to our mailing list, which is also kept with Resend, and Resend sends our emails. Only your email address and the time you agreed are sent. | `src/lib/copy/en.ts:308` | paragraph |
| P14 | This site is hosted by Netlify. [FILL IN: for the lawyer: describe what technical details of a visit, such as the IP address, the hosting provider may record] | `src/lib/copy/en.ts:309` | paragraph |

### 2.6 How long we keep it

| ID | Text | Where | Shown when |
|---|---|---|---|
| P15 | How long we keep it | `src/lib/copy/en.ts:313` | section heading |
| P16 | [FILL IN: how long the email address and consent time are kept] | `src/lib/copy/en.ts:314` | paragraph |

### 2.7 How to unsubscribe or have your email address deleted

| ID | Text | Where | Shown when |
|---|---|---|---|
| P17 | How to unsubscribe or have your email address deleted | `src/lib/copy/en.ts:317` | section heading |
| P18 | Use the unsubscribe link in every email you receive. [FILL IN: confirm that emails sent through Resend include an unsubscribe link] | `src/lib/copy/en.ts:319` | paragraph |
| P19 | You can also write to [FILL IN: email address for requests] to ask us to delete your email address. | `src/lib/copy/en.ts:320` | paragraph |

### 2.8 Your rights

| ID | Text | Where | Shown when |
|---|---|---|---|
| P20 | Your rights | `src/lib/copy/en.ts:324` | section heading |
| P21 | [FILL IN: for the lawyer: the user's rights under the law, and how to use them] | `src/lib/copy/en.ts:325` | paragraph |

### 2.9 Contact

| ID | Text | Where | Shown when |
|---|---|---|---|
| P22 | Contact | `src/lib/copy/en.ts:328` | section heading |
| P23 | [FILL IN: name of the site operator] · [FILL IN: contact email address] | `src/lib/copy/en.ts:330` | paragraph |
...
| T03 | Privacy Policy (DRAFT) · Tunay na Interes | `src/lib/copy/en.ts:334` | tab title, privacy page while draft |
| T04 | Privacy Policy · Tunay na Interes | `src/lib/copy/en.ts:335` | tab title, privacy page once final |
...
| ID | Text | Where | Shown when |
|---|---|---|---|
| E01 | Something went wrong | `src/lib/copy/en.ts:337` | heading |
| E02 | An unexpected error occurred. Try reloading the page. | `src/lib/copy/en.ts:338` | message when the error has no text of its own; otherwise the error's own message is shown |
